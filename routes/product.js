...
router.get("/get", auth.authenticateToken, (reg, res, next) => {
  var query =
    "select p.id,p.name,p.description,p.price,p.status,c.id  as categoryName from product as p INNER JOIN category as c where p.categoryId = c.id";
      ...
      (err, results) => {
        if (!err) {
          if (results.affectedRows == 0) {
            return res
              .status(404)
              .json({ message: "product id was not found" });
          }
          return res
            .status(200)
